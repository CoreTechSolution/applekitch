...
function pdf_to_thumbnail($abs_path='uploads', $destination){
    $rtntext='';
    $CI = & get_instance();
    $CI->load->library('upload');

    $pdf_file_name=explode('/',$abs_path);
    ...
    if(file_exists($abs_path) ){
        ...
        $image_name     = $pdf_file_name_without_ext[0]."_$ig".'.jpg';

        $im->writeImage($destination.'/'.$image_name);
        $im->clear();
        $im->destroy();
        $rtntext = $rtntext=base_url().$destination.'/'.$image_name;
