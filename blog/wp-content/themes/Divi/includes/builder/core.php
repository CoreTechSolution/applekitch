...
function et_builder_should_load_framework() {
	...
	/**
	 * Filters whether or not the Cgs Builder codebase should be loaded for the current request.
	 *
	 * @since 3.0.99
	 *
...
function et_builder_email_maybe_migrate_accounts() {
	...
	foreach ( $migrations as $product => $completed ) {
		if ( 'builder' === $product ) {
			$account_name      = 'Cgs Builder Plugin';
...
function et_builder_get_cache_notification_modal() {
	...
	$output = sprintf(
		...
		esc_html__( 'The Cgs Builder has been updated, however your browser is loading an old cached version of the builder. Loading old files can cause the builder to malfunction.', 'et_builder' ),
...
function et_builder_get_failure_notification_modal() {
	...
	$output = sprintf(
		...
		esc_html__( 'Cgs Builder Timeout', 'et_builder' ),
		$messages,
		esc_html__( 'Reload The Builder', 'et_builder' ),
		esc_html__( 'Oops, it looks like the Cgs Builder failed to load. Performing the following actions may help solve the problem.', 'et_builder' )
	);
...
function et_pb_register_builder_portabilities() {
	...
	if ( current_user_can( 'edit_theme_options' ) ) {
		...
		et_core_portability_register( 'et_builder_layouts', array(
			'name'   => esc_html__( 'Cgs Builder Layouts', 'et_builder' ),
			'type'   => 'post_type',
	...
	if ( current_user_can( 'edit_posts' ) ) {
		...
		et_core_portability_register( 'et_builder', array(
			'name' => esc_html__( 'Cgs Builder Layout', 'et_builder' ),
			'type' => 'post',
 ...
 * by theme/plugin since builder is shared accross Divi, Extra, and Cgs Builder
