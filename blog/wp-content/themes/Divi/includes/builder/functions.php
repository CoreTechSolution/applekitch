...
// call the widgets init at 'init' hook if Cgs Builder plugin active
// this is needed because plugin loads the Cgs Builder at 'init' hook and 'widgets_init' is too early.
if ( et_is_builder_plugin_active() ) {
...
function et_pb_before_main_editor( $post ) {
	...
	if ( et_pb_is_allowed( 'divi_builder_control' ) ) {
		$buttons = sprintf('<a href="#" id="et_pb_toggle_builder" data-builder="%2$s" data-editor="%3$s" class="button button-primary button-large%4$s%5$s">%1$s</a>',
			( $is_builder_used ? esc_html__( 'Use Default Editor', 'et_builder' ) : esc_html__( 'Use The Cgs Builder', 'et_builder' ) ),
			esc_html__( 'Use The Cgs Builder', 'et_builder' ),
...
function et_pb_add_custom_box() {
	...
	foreach ( $post_types as $post_type ){
		add_meta_box( ET_BUILDER_LAYOUT_POST_TYPE, esc_html__( 'The Cgs Builder', 'et_builder' ), 'et_pb_pagebuilder_meta_box', $post_type, 'normal', 'high' );
	}
}
...
function et_pb_pagebuilder_meta_box() {
	...
	printf(
		...
		esc_html__( 'All content created in the Cgs Builder will be lost. Previous content will be restored.', 'et_builder' ),
	...
	printf(
		...
		esc_html__( 'Cgs Builder Settings', 'et_builder' ),
	...
	printf(
		...
		esc_html__( 'Cgs Builder Helper', 'et_builder' ),
		...
		'builder_capabilities' => array(
			...
			'options'       => array(
				...
				'divi_builder_control' => array(
					'name'    => esc_html__( 'Toggle Cgs Builder', 'et_builder' ),
				),
				'load_layout' => array(
...
function et_fb_get_product_tour_text( $post_id ) {
	...
	$productTourText = array(
		'start' => array(
			'title' => esc_html__( 'Welcome To The Cgs Builder', 'et_builder' ),
			'description' => sprintf(
		...
		'addSection' => array(
			...
			'description' => sprintf(
				__( 'Now that your pre-made layout has been loaded, we can start adding new content to the page. The Cgs Builder organizes content using %1$s, %2$s and Modules. Sections are the largest organizational element. Click the highlighted button to add a new section to the page.', 'et_builder' ),
		...
		'selectModule' => array(
			...
			'description' => esc_html__( 'Within each column you can add one or more Modules. A module is basic content element. The Cgs Builder comes with over 40 different content elements to choose from, such as Images, Videos, Text, and Buttons. Click the highlighted Blurb button to add a new Blurb module to the first column in your row.', 'et_builder' ),
		),
		...
		'saveModule' => array(
			...
			'description' => esc_html__( 'Whenever you make changes in the Cgs Builder, these changes can be Undone, Redone, Discarded or Accepted. Now that you have adjusted your module’s title, you can click the red discard button to cancel these changes, or your can click the green button to accept them.', 'et_builder' ),
		),
		'duplicateModule' => array(
			'title' => esc_html__( 'Hover To Access Action Buttons', 'et_builder' ),
			'description' => esc_html__( 'Whenever you hover over a Section, Row or Module in the Cgs Builder, action buttons will appear. These buttons can be used to move, modify, duplicate or delete your content. Click the highlighted “duplicate” icon to duplicate the blurb module that you just added to the page.', 'et_builder' ),
		),
		'moveModule' => array(
		...
		'pageSettings' => array(
			...
			'description' => esc_html__( 'While using the Cgs Builder, you can access your page settings by toggling the page settings bar at the bottom of your screen. Click the highlighted button to reveal your page settings.', 'et_builder' ),
		),
		'tabletPreview' => array(
...
function et_fb_process_shortcode( $content, $parent_address = '', $global_parent = '', $global_parent_type = '' ) {
	...
	foreach ( $matches as $match ) {
		...
		// Add dummy object to render it in Cgs Builder
